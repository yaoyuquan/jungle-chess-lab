...
| 情形 | 谁兜的 | 表现 |
|---|---|---|
| 模型返回越界编号 / 调用失败 / 没配密钥 | 后端启发式（入巢 > 吃高价值子 > 向对方兽巢推进） | 200 + `fallback: true` |
| 后端整个不可用（没起、断网） | 前端就地随机落子（`useGame`） | 记录里写明「AI 服务不可用」 |
| 棋手清单拉不到 | 前端内置清单（`useModels.FALLBACK_PLAYERS`） | 下拉框照常可选 |

对局记录里凡是兜底的一手都带「兜底」角标。**后端不起也能完整游玩**，只是 AI 变成随机落子。
...
想让 AI 真正由模型来下，另起后端（见 chess-lab-server 的 README）：
